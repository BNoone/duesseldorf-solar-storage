...
- The Stadtteil layer: filled polygons coloured by total roof potential (kWp), a quantile-binned sequential scale (ColorBrewer Oranges, 5 classes) rather than a linear one, since Bilk's 89,536 kWp against Knittkuhl's 3,863 kWp would flatten a linear scale's middle. A legend shows the actual kWp value at each bin edge. Every Stadtteil shows its name on hover; the 15 largest by area also carry a permanent label, so the map reads as named neighbourhoods without needing to hover each one. Clicking opens a popup with qualifying buildings, roof potential, annual yield, and battery potential. The header totals, the realization rate, and the footer pull date are read live from `data/stadtteile.json`, not hardcoded.
